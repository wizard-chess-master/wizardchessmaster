...
- **Board Alignment Fixes (Aug 14 2025)**: Fixed right border visibility by properly accounting for 8px total border width in calculations, reduced top margin from -50px to -30px to minimize gap, adjusted sizeFactor to 0.75 for better viewport fit, updated maxWidth to calc(100vw - 48px) and maxHeight to calc(100vh - 100px) for proper positioning
- **Performance & Display Fixes (Aug 14 2025)**: Fixed board positioning (removed negative margin), improved board sizing calculation for better visibility, optimized console logging to reduce FPS drops, fixed FPS monitoring to limit warnings, verified ElevenLabs API integration working properly
