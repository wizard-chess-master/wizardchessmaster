/**
 * Test control tags generation with sample game states
 */
...
// Create test moves
function createTestMove(): ChessMove {
  return {
    from: { row: 8, col: 4 },
    to: { row: 6, col: 4 },
...
export async function testControlTags() {
  ...
  try {
    ...
    const captureMove = { ...move, captured: { type: 'pawn', color: 'black' } };
    ...
    for (const quality of qualities) {
      // Mock different quality scores by adjusting the move
      const testMove = quality > 0.5 ? 
        { ...move, captured: { type: 'queen', color: 'black' } } : 
        move;
      const tags = await aiCoach.addControlTags(gameState, testMove);
      console.log(`Quality ~${quality}: "${tags.suggestion}"`);
    }
...
if (typeof window !== 'undefined') {
  ...
  console.log('💡 Run testControlTags() in console to test control tags');
}