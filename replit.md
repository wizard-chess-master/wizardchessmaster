...
- **Wizard Check Validation Bug** (August 11, 2025): Fixed critical bug where kings couldn't escape from wizard checks. The move validation logic was incorrectly simulating wizard attacks - it was moving the wizard to the target square instead of keeping it in place. Corrected the test board simulation to properly handle wizard attacks (wizard stays in place, only target is removed).
